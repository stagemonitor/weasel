...
export function instrumentXMLHttpRequest() {
  ...
  function InstrumentedXMLHttpRequest() {
    ...
    xhr.open = function open(method, url, async) {
      ...
      var sampled = Math.random() <= vars.sampleRate ? 1 : 0;
      beacon['t'] = traceId;
      beacon['s'] = spanId;
      beacon['m'] = method;
...
function isSameOrigin(url: string): bool {
  ...
  try {
    ...
    return (urlAnalysisElement.protocol && urlAnalysisElement.protocol !== ':' ? urlAnalysisElement.protocol : loc.protocol) === loc.protocol &&
      (urlAnalysisElement.hostname || loc.hostname) === loc.hostname &&
      (urlAnalysisElement.port || loc.port) === loc.port;
  } catch (e) {
    return false;
  }
}
